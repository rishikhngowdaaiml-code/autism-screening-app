...
export const storage = {
  ...
  // Logout helper: removes only the authenticated user (keeps language and data by default)
  logout: () => {
    localStorage.removeItem("current_user");
    // If you want to fully clear all user data on logout, uncomment these lines:
    // const user = storage.currentUser();
    // const phone = user?.phone || 'anon';
    // localStorage.removeItem(CHILDREN_KEY(phone));
    // localStorage.removeItem(REPORTS_KEY(phone));
  }
};